...
RSpec.describe "prometheus recording rules" do
  prom_alerts = nil
  ...
  Dir.glob("*.test.yml", base: "spec/recording-rules/") do |filename|
    it "runs #{filename}" do
      _, err, status = Open3.capture3("promtool test rules spec/recording-rules/#{filename}")
